...
int WBM (int *Map) {
  ...
  int *q = (int *) malloc(sizeof(int) * minRegNu);
  ...
  do {
    ...
    for (i = 0; i < minRegNu; i++) {
      ...
      if (p[i] != NULL) {
        ...
        int flag = 0;
        //if (clus_nu > 1) {
          // Search all the register entries expect itself in the VarTable
          for (j = 0; j < minRegNu; j++) {
            if (i != j && p[j] !=NULL && p[j]->next != NULL) {
              if (p[i]->startTime > p[j]->next->startTime) {
        ...
        if (!flag) {
          ...
          q[i] = 1; // remember the pointer which needs to be advanced
        }
      }
    ...
    // update pointer due to register search above
    for (i = 0; i < minRegNu; i++) {
      if (q[i]) p[i] = p[i]->next; 
    }
  ...
  for (i = 0; i < minRegNu; i++) {
    ...
    if (var_MUXP_table[i].var_list != NULL) {
      for (n = 0; n < var_MUXP_table[i].var_nu; n++) {
        myprintf("(%d %c %d) ", \
            (var_MUXP_table[i].var_list[n]).varID,\
   ...
   for (clus_nu = 1; clus_nu < clusterNu; clus_nu++) {
     ...
     for (j = 0; j < minRegNu; j++) {
       ...
       if (setV[clus_nu][j].varID != -1) {
         ...
         // Check if vj can be assigned to any register
         conflict_nu = 0;
  ...
  // Free VarList
  myprintf("-- Print sorted variables' life time(To be deleted) --\n");
