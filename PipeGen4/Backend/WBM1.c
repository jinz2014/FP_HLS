...
int WBM1 (int *Map) {
  ...
  // 
  varList *q = (varList *) malloc(sizeof(varList) * minRegNu);
    ...
    for (i = 0; i < minRegNu; i++) {
      ...
      if (p[i] != NULL) {
        ...
        if (first) {
          ...
          first = 0;
          for (j = 0; j < i; j++) // for all i's which make p[i] = NULL
            setV[clus_nu - 1][j].varID = -1;
        }

        for (j = 0; j < i; j++) {
          if (setV[clus_nu - 1][j].varID != -1 &&
              setV[clus_nu - 1][j].startTime > p[i]->startTime) {
            setV[clus_nu - 1][j].varID = -1;
            p[j] = q[j];
          }
        }
